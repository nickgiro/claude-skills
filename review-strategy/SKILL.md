...
- **Product-side mirror** – When the doc lists user-side conditions for value (e.g. "users derive value when they articulate a behavior / trust the AI / notice change / wear consistently"), does it also include the product-side mirror that the team actually controls (e.g. "product elicits the behavior / earns trust via transparency / surfaces change early / is wearable day 1")? User-side-only framing produces a selection mindset instead of a design mindset, blurs the locus of failure, and tends toward tautology. The product-side mirror is where the team's load-bearing design work lives – and the most strategic condition is usually the one where that mirror is hardest, not where the user-side condition is hardest.
